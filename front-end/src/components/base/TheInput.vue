...
<template>
	<div class="the-input">
		<div class="the-input__container">
			...
			<div class="the-input__input-box input-box" :class="{ 'input-box--error': errorMessage }">
				<TheIcon class="input-box__icon" v-if="iconName" :name="iconName" />
				<textarea v-if="type === 'textarea'" class="input-box__input" style="height: 80px; resize: vertical"
					v-model="inputValue" :placeholder="placeholder" :disabled="isDisabled" @input="handleInput"
					@blur="emit('blur', inputValue)" @focus="emit('focus', inputValue)" />
				<input v-else class="input-box__input" v-model="inputValue" :placeholder="placeholder" :disabled="isDisabled"
					:type="inputType" @input="handleInput" @blur="emit('blur', inputValue)" @focus="emit('focus', inputValue)" />
				<TheIcon v-if="isPassword" class="input-box__eye-icon" :name="eyeIconName" @click="togglePasswordVisibility" />
			</div>
